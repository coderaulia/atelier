// Quick Quote Calculator — panel + preview
// Globals from utils.jsx: fmt, Field, TextInput, Textarea, SectionTitle

const { useState: _useStateQC } = React;

function QuoteCalculatorPanel({ data, onChange }) {
  const set = (k, v) => onChange({ ...data, [k]: v });
  const [copied, setCopied] = _useStateQC(false);
  ...
  return (
    <>
      ...
      <Field label="Service description">
        <TextInput value={data.serviceDescription} onChange={v => set("serviceDescription", v)} placeholder="Brand Identity Design" />
      </Field>
      <Field label="Currency">
        <select className="field__select" value={currency} onChange={e => set("currency", e.target.value)}>
      ...
      <div className="field__row">
        <Field label="Hours">
          <TextInput type="number" value={data.hours} onChange={v => set("hours", v)} placeholder="40" />
        </Field>
        <Field label={`Rate per hour (${currency})`}>
          <TextInput type="number" value={data.hourlyRate} onChange={v => set("hourlyRate", v)} placeholder="180" />
        </Field>
      </div>
      ...
      <div className="field__row">
        <Field label="Discount %">
          <TextInput type="number" value={data.discountPct} onChange={v => set("discountPct", v)} placeholder="0" />
        </Field>
        <Field label="Tax %">
          <TextInput type="number" value={data.taxPct} onChange={v => set("taxPct", v)} placeholder="0" />
        </Field>
      </div>
...
Object.assign(window, { QuoteCalculatorPanel, QuotePreview });
